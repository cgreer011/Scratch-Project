...
maincontroller.saveUrl = (req, res, next) => {
  ...
  db.query(updateUrlTable, [userId, req.body.url])//recieved from state
    .then((saved) => {
      res.locals.urlId = saved.rows[0].urlId
      next();
...
maincontroller.pingUrl = (req, res, next) => {
  if (!res.locals.url) {
    res.locals.url = req.body.url;//recieved from state
    res.locals.urlId = req.body.url_id;//recieved from state
  }
